...
describe("Card", () => {
  ...
  beforeEach(async () => {
    ...
    deployedContract = await deployTestContract("Card");
  });

  async function mintNftDefault(): Promise<TransactionResponse> {
    return deployedContract.draw(wallet.address, TOKEN_URI);
  }
  ...
  describe("mintNft", async () => {
    ...
    it("returns the new item ID", async () => {
      await expect(
        await deployedContract.callStatic.draw(wallet.address, TOKEN_URI)
      ).to.eq("1");
    });
    ...
    it("cannot mint to address zero", async () => {
      const TX = deployedContract.draw(
        ethers.constants.AddressZero,
        TOKEN_URI
      );
